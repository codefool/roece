...
// Results from board::seek
enum SeekResultCode
{
    SEEKRC_NONE,          // nothing found
    SEEKRC_FOUND_ENEMY,   // found an enemy
    SEEKRC_FOUND_FRIENDLY // found a friendly
};
...
// SeekResult
//
// Contains the results of a board::seek attempt
//   rc   - Result of the seek (found nothing/enemy/friend)
//   path - the squares passed through to the result
//   occ  - occupant of the square
struct SeekResult
{
    SeekResultCode rc;
    SquareList     path;
...
struct MoveResult
{
    ...
    PiecePtr ppSrc;    // piece being moved
    PiecePtr ppTrg;    // optional affected piece (capture)
    BoardState bs;     // board state
    Square sqSrc;      // where ppSrc ended up
    Square sqTrg;      // where ppTrg ended up
    MoveResult();
